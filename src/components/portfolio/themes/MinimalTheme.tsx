...
import { Star, ExternalLink, Globe, MonitorPlay, FileDown } from 'lucide-react'
...
interface Props {
  ...
  portfolio: { 
    bio: string | null; 
    skills: string[]; 
    contactEmail: string | null; // FIX: Added contactEmail to TS interface
    twitter: string | null;
...
export default function MinimalTheme({ user, portfolio }: Props) {
  // FIX: Renamed from 'featured' to 'featuredProjects'
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans print:bg-white print:text-black">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 space-y-12 print:py-0 print:space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start gap-6">
          {user.avatar && (
            <img src={user.avatar} alt={user.name ?? ''} className="w-20 h-20 rounded-full border-4 border-gray-100 print:border-gray-300" />
          )}
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-gray-900">{user.name}</h1>
            <p className="text-gray-500 mt-1">@{user.username}</p>
            {portfolio.bio && <p className="text-gray-700 mt-3 max-w-xl print:text-black">{portfolio.bio}</p>}
        ...
        {portfolio.skills.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-4 print:text-gray-500">Skills</h2>
            <div className="flex flex-wrap gap-2">
              {portfolio.skills.map((skill) => (
                <span key={skill} className="bg-gray-100 text-gray-700 text-sm px-3 py-1 rounded-full print:border print:border-gray-300 print:bg-transparent">
                  {skill}
                </span>
              ))}
            </div>
        ...
        {sections.length > 0 && (
          <div className="space-y-10">
            {sections.map((section) => (
              <div key={section.id}>
                <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-6 print:text-gray-500">{section.title}</h2>
                <div className="space-y-6">
                  {section.items.map((item) => (
                    <div key={item.id} className="relative pl-6 before:absolute before:left-0 before:top-2 before:bottom-0 before:w-[2px] before:bg-gray-100 print:before:bg-gray-300 print:break-inside-avoid">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-4">
                        ...
                        {item.date && (
                          <span className="text-sm text-gray-400 whitespace-nowrap print:text-gray-600">{item.date}</span>
                        )}
                      </div>
                      {item.subtitle && <p className="text-sm font-medium text-gray-600 mt-0.5">{item.subtitle}</p>}
                      {item.description && <p className="text-sm text-gray-500 mt-2 whitespace-pre-wrap print:text-black">{item.description}</p>}
                    </div>
                  ))}
                </div>
              </div>
        ...
        {featuredProjects.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-4 print:text-gray-500">Selected Projects</h2>
            <div className="space-y-4">
              {featuredProjects.map((project) => (
                <div key={project.id} className="border border-gray-200 rounded-xl overflow-hidden print:border-gray-300 print:break-inside-avoid">
                  ...
                  <div className="p-5">
                    ...
                    {project.description && <p className="text-gray-500 text-sm mt-2 print:text-black">{project.description}</p>}
